...
class TestCreateSession:
    ...
    async def test_create_session(self, session_manager):
        ...
        session = await session_manager.create_session("test", "gpt-4", "openai")
    ...
    @pytest.mark.asyncio
    async def test_create_session_sanitizes_name(self, session_manager):
        """Test that session names are sanitized."""
        session = await session_manager.create_session("test/name:with*chars", "gpt-4", "openai")
    ...
    async def test_create_multiple_sessions(self, session_manager):
        ...
        session1 = await session_manager.create_session("session1", "gpt-4", "openai")
        session2 = await session_manager.create_session("session2", "claude-3", "anthropic")
...
class TestLoadSession:
    ...
    async def test_load_session(self, session_manager):
        ...
        await session_manager.create_session("test", "gpt-4", "openai")
    ...
    async def test_load_nonexistent_session(self, session_manager):
        ...
        loaded = await session_manager.load_session("nonexistent")
        assert loaded is None

    @pytest.mark.asyncio
    async def test_load_session_with_turns(self, session_manager):
        """Test loading a session with conversation turns."""
        session = await session_manager.create_session("test", "gpt-4", "openai")
        turn = Turn(role="user", content="Hello", model="gpt-4", tokens={"total": 10})
        await session_manager.add_turn(session, turn)
...
class TestGetOrCreateSession:
    ...
    async def test_get_or_create_new(self, session_manager):
        ...
        session = await session_manager.get_or_create_session("new", "gpt-4", "openai")
    ...
    async def test_get_or_create_existing(self, session_manager):
        ...
        await session_manager.create_session("existing", "gpt-4", "openai")
        session = await session_manager.get_or_create_session("existing", "claude", "anthropic")
...
class TestAddTurn:
    ...
    @pytest.mark.asyncio
    async def test_add_turn_with_tokens(self, session_manager):
    ...
    @pytest.mark.asyncio
    async def test_add_multiple_turns(self, session_manager):
        ...
        await session_manager.add_turn(session, turn1)
        await session_manager.add_turn(session, turn2)
...
class TestSaveSession:
    ...
    @pytest.mark.asyncio
    async def test_save_updates_updated_at(self, session_manager):
...
class TestDeleteSession:
    ...
    async def test_delete_session(self, session_manager):
        ...
        assert await session_manager.load_session("test") is None
...
class TestRenameSession:
    ...
    async def test_rename_session(self, session_manager):
        ...
        await session_manager.create_session("old", "gpt-4", "openai")
        assert await session_manager.rename_session("old", "new") is True
        assert await session_manager.load_session("old") is None
        assert await session_manager.load_session("new") is not None

    @pytest.mark.asyncio
    async def test_rename_to_existing(self, session_manager):
        """Test renaming to an existing session name fails."""
        await session_manager.create_session("a", "gpt-4", "openai")
        await session_manager.create_session("b", "gpt-4", "openai")
        assert await session_manager.rename_session("a", "b") is False
        # Original should still exist
        assert await session_manager.load_session("a") is not None

    @pytest.mark.asyncio
    async def test_rename_nonexistent(self, session_manager):
        """Test renaming a session that doesn't exist fails."""
        assert await session_manager.rename_session("nonexistent", "new") is False

    @pytest.mark.asyncio
    async def test_rename_preserves_content(self, session_manager):
        """Test that rename preserves session content."""
        session = await session_manager.create_session("old", "gpt-4", "openai")
        turn = Turn(role="user", content="Hello", model="gpt-4")
        await session_manager.add_turn(session, turn)
        await session_manager.save_session(session)

        await session_manager.rename_session("old", "new")
        loaded = await session_manager.load_session("new")
        assert len(loaded.turns) == 1
        assert loaded.turns[0].content == "Hello"
...
class TestListSessions:
    ...
    async def test_list_sessions(self, session_manager):
        ...
        await session_manager.create_session("a", "gpt-4", "openai")
        await session_manager.create_session("b", "claude", "anthropic")
        sessions = await session_manager.list_sessions()
        assert len(sessions) == 2

    @pytest.mark.asyncio
    async def test_list_sessions_excludes_temp(self, session_manager):
        """Test that temp sessions are excluded from list."""
        await session_manager.create_session("regular", "gpt-4", "openai")
        await session_manager.get_temp_session("gpt-4", "openai")
        sessions = await session_manager.list_sessions()
        assert len(sessions) == 1
        assert sessions[0].name == "regular"

    @pytest.mark.asyncio
    async def test_list_sessions_sorted_by_updated(self, session_manager):
        """Test that sessions are sorted by updated_at descending."""
        await session_manager.create_session("first", "gpt-4", "openai")
        time.sleep(0.01)
        await session_manager.create_session("second", "gpt-4", "openai")
...
class TestSearchSessions:
    ...
    @pytest.mark.asyncio
    async def test_search_by_name(self, session_manager):
        """Test searching sessions by name."""
        await session_manager.create_session("quantum-research", "gpt-4", "openai")
        await session_manager.create_session("other", "gpt-4", "openai")
        results = await session_manager.search_sessions("quantum")
        assert len(results) == 1
        assert results[0].match_type == "name"
        assert results[0].session_name == "quantum-research"

    @pytest.mark.asyncio
    async def test_search_by_content(self, session_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_case_insensitive(self, session_manager):
        """Test that search is case insensitive."""
        await session_manager.create_session("QUANTUM", "gpt-4", "openai")
        results = await session_manager.search_sessions("quantum")
        assert len(results) == 1
    ...
    async def test_search_no_results(self, session_manager):
        ...
        await session_manager.create_session("test", "gpt-4", "openai")
        results = await session_manager.search_sessions("xyz")
        assert len(results) == 0

    @pytest.mark.asyncio
    async def test_search_excludes_temp_sessions(self, session_manager):
        """Test that temp sessions are excluded from search."""
        temp = await session_manager.get_temp_session("gpt-4", "openai")
        turn = Turn(role="user", content="quantum test", model="gpt-4")
        await session_manager.add_turn(temp, turn)
        await session_manager.save_session(temp)

        results = await session_manager.search_sessions("quantum")
        assert len(results) == 0

    @pytest.mark.asyncio
    async def test_search_name_takes_priority(self, session_manager):
        """Test that name match takes priority over content match."""
        session = await session_manager.create_session("quantum", "gpt-4", "openai")
...
class TestTempSession:
    ...
    @pytest.mark.asyncio
    async def test_temp_session(self, session_manager):
        """Test creating a temporary session."""
        temp = await session_manager.get_temp_session("gpt-4", "openai")
        assert temp.name.startswith(".temp-")

    @pytest.mark.asyncio
    async def test_temp_session_unique(self, session_manager):
        """Test that temp sessions have unique names."""
        temp1 = await session_manager.get_temp_session("gpt-4", "openai")
        time.sleep(0.002)  # Ensure different timestamp
        temp2 = await session_manager.get_temp_session("gpt-4", "openai")
...
class TestCleanupTempSessions:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_keeps_recent_temp_sessions(self, session_manager):
...
class TestExportSession:
    ...
    @pytest.mark.asyncio
    async def test_export_json(self, session_manager):
        """Test exporting session as JSON."""
        await session_manager.create_session("test", "gpt-4", "openai")
        content = await session_manager.export_session("test", "json")
        assert '"name": "test"' in content
        assert '"model": "gpt-4"' in content

    @pytest.mark.asyncio
    async def test_export_markdown(self, session_manager):
        ...
        content = await session_manager.export_session("test", "markdown")
        assert "# Session: test" in content
        assert "Hello" in content
        assert "**Model:** gpt-4" in content

    @pytest.mark.asyncio
    async def test_export_text(self, session_manager):
        ...
        content = await session_manager.export_session("test", "text")
        assert "Session: test" in content
        assert "Hello" in content
        assert "[User]" in content

    @pytest.mark.asyncio
    async def test_export_nonexistent_session(self, session_manager):
        """Test exporting a session that doesn't exist."""
        with pytest.raises(ValueError) as excinfo:
            await session_manager.export_session("nonexistent", "json")
        assert "not found" in str(excinfo.value).lower()

    @pytest.mark.asyncio
    async def test_export_invalid_format(self, session_manager):
        """Test exporting with invalid format."""
        await session_manager.create_session("test", "gpt-4", "openai")
        with pytest.raises(ValueError) as excinfo:
            await session_manager.export_session("test", "invalid")
